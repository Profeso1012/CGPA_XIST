    ...
    setFutureSemesters(
      futureSemesters.map((sem) => {
        if (sem.id === semesterId) {
          ...
          // If updating units, validate the total units
          if (field === "units") {
            validateCourseTNU(semesterId, updatedCourses, sem.tnu)
          }
    ...
    <div className="space-y-6">
      ...
      >
        <TabsList className="mb-4 bg-gray-100 dark:bg-gray-800">
          <TabsTrigger value="semester" className="data-[state=active]:bg-cgpa-blue data-[state=active]:text-white">
            Semester-based Simulation
          </TabsTrigger>
          <TabsTrigger value="course" className="data-[state=active]:bg-cgpa-blue data-[state=active]:text-white">
        ...
        <TabsContent value="semester">
          <motion.div
            ...
            transition={{ duration: 0.5 }}
          >
            <div className="grid grid-cols-12 gap-4 font-medium text-sm bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
              <div className="col-span-5">Semester</div>
              <div className="col-span-2">TNU</div>
              <div className="col-span-2">TCP</div>
            ...
            {futureSemesters.map((semester, index) => (
              <motion.div
                key={semester.id}
                className="grid grid-cols-12 gap-4 items-center bg-white dark:bg-gray-900 p-3 rounded-lg shadow-sm"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
          ...
          {futureSemesters.map((semester, semIndex) => (
            ...
            >
              <Card className="mb-6 border-cgpa-blue/20 overflow-hidden">
                <CardHeader className="flex flex-row items-center justify-between pb-2 bg-gradient-to-r from-cgpa-blue/10 to-transparent">
                  <CardTitle className="text-lg text-cgpa-blue">
                    <Input
                      value={semester.name}
                      onChange={(e) => {
                        const updated = [...futureSemesters]
                ...
                <CardContent>
                  ...
                  {semester.courses.length > 0 ? (
                    <div className="space-y-4">
                      <div className="grid grid-cols-12 gap-4 font-medium text-sm bg-gray-50 dark:bg-gray-800 p-2 rounded-lg">
                        <div className="col-span-4 sm:col-span-3">Course Code</div>
                        <div className="col-span-2 sm:col-span-2">Units</div>
                        <div className="col-span-3 sm:col-span-3">Grade Point (0-5)</div>
                        <div className="col-span-2 sm:col-span-3">Credit Point</div>
                        <div className="col-span-1 sm:col-span-1"></div>
                      </div>

                      {semester.courses.map((course, courseIndex) => (
                        <div key={courseIndex} className="grid grid-cols-12 gap-4 items-center">
                          ...
                          <div className="col-span-2 sm:col-span-2">
                            <Input
                              type="number"
                              value={course.units || ""}
                              onChange={(e) => updateCourse(semester.id, courseIndex, "units", Number(e.target.value))}
                              min="0"
                              max="6"
                              className="border-cgpa-blue/20 focus:border-cgpa-blue focus:ring-cgpa-blue"
                            />
                          ...
                          <div className="col-span-3 sm:col-span-3">
                            <Input
                              ...
                              onChange={(e) => {
                                const gradePoint = Math.min(5, Math.max(0, Number(e.target.value)))
                                updateCourse(semester.id, courseIndex, "gradePoint", gradePoint)

                                // Update grade based on grade point
                                let grade = ""
                                if (gradePoint >= 4.5) grade = "A"
                                else if (gradePoint >= 3.5) grade = "B"
                                else if (gradePoint >= 2.5) grade = "C"
                                ...
                                updateCourse(semester.id, courseIndex, "grade", grade)
                              }}
                              min="0"
                              max="5"
                              step="0.1"
                          ...
                          <div className="col-span-2 sm:col-span-3">
                            <Input
                              ...
                              readOnly
                              className="bg-gray-50 dark:bg-gray-800 font-medium text-cgpa-gold"
                            />
                          </div>
